...
names (e.g. "ATL" -> "America/New_York"), sourced from the OpenFlights
airport database. This only needs to be run once (or occasionally, if you
add routes to/from an airport not already in the table).
...
def build_lookup() -> pd.DataFrame:
    ...
    lookup = lookup[["iata", "tz_name"]].drop_duplicates(subset="iata")
